...
class NodeAgentBackchannel(AgentBackchannel):
    def __init__(
        self, 
        ident: str,
        http_port: int,
    ...
    async def start_node(self):
        ...
        print("Start node agency process")
        ...
        input_config = "node/node_agency/localhost.env.template"
        self.output_config = "/aries-backchannels/vcxagencynode/vcxagency-node/config/localhost.env"
        ...
        await self.start_node_agency()

        print("Provision an agent and wallet, get back configuration details")
        provisionConfig["agency_url"] = "http://localhost:" + str(self.http_port)
#        provisionConfig["agency_url"] = "http://host.docker.internal:" + str(self.http_port)
    ...
    async def make_agent_POST_request(
        ...
        if op["topic"] == "connection":
            ...
            if operation == "create-invitation":
                ...
                await connection.connect('{"use_public_did": false}')
...
async def main(start_port: int, show_timing: bool = False, interactive: bool = True):
    ...
    try:
        agent = NodeAgentBackchannel(
            "node", start_port+1, start_port+2, genesis_data=genesis
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Runs a node demo agent.")
    parser.add_argument(
        "-p",
        "--port",
        type=int,
